...
**Status:** Phases 1–3 shipped (pending-handoff record, `HandoffReconciler`, `portage orders reconcile`, `handoff_spend_mode`, `portage buy --wait`/`--wait-timeout` with NDJSON events, and the `webhook`/`macos`/`terminal` notify channels via `reconcile_notify`). Phase 0's live signal check was **not run** — no live store credentials available in that session — so Phases 1–3 were built on the plan's own default assumption (`completed` is observable) rather than a confirmed one; see design-log §44 before relying on this against a real store.
Phase 4 is partially shipped: `webmcp_checkout_mode` (default `express_stop`) and `Buy#webmcp_flow` wire an optional, injected `portage-ucp-webmcp` outbound bridge into `Buy`'s discovery order (opt-in via `Buy.new(webmcp_bridge:)` — no CLI flag, since a shell invocation has no browser to hand it one). `express_stop` builds the cart/checkout and always hands off, feeding Phases 1–3 unchanged. `token` mode is a deliberate refusal (`webmcp_token_unsupported`), not an implementation: it still needs the `Mcp::Server.build` Confirmer-swap seam and PayPal/Stripe agent-token enrollment this plan always said were prerequisites.
