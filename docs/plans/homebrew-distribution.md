...
1. **Release first.** Publish `portage-cli` (with `--version` and `irb`), plus `portage-ucp-client` 0.6.3, `portage-ucp-decision` 0.1.1 and `portage-ucp-webmcp` 0.1.1, all of which are bumped on `main` but not yet on rubygems, via `rake publish_all`. Until then the generator's default mode refuses to run, and a `--latest-published` formula fails `brew test`.
