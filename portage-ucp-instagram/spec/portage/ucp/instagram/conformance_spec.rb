...
RSpec.describe Portage::Ucp::Instagram::Adapter do
  ...
  # The product fetch is the only network call the kit reaches for checkout:
  ...
  # so Instagram's #complete_checkout NotImplementedError is never what makes
  # it pass. The catalog reads are hit directly by the kit's own
  ...
  before do
    ...
    stub_request(:get, "https://graph.facebook.com/v21.0/1?fields=id,name,description,price,availability,url,item_group_id")
      .to_return(status: 200, body: product_node.to_json)
    stub_request(:get, %r{/catalog_1/products\?fields=id,name,description,price,availability,url,item_group_id})
