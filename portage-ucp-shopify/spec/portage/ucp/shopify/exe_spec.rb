...
RSpec.describe "exe/portage-ucp-shopify" do
  let(:gem_root) { File.expand_path("../../../..", __dir__) }
  let(:exe_path) { File.join(gem_root, "exe", "portage-ucp-shopify") }
  let(:base_env) do
  ...
  # answers requests, without needing a real Shopify store.
  ...
  it "raises a clear error when a required env var is missing" do
    _stdout, stderr, status = run_exe({ "SHOPIFY_SHOP_DOMAIN" => nil }, stdin_data: "")

    expect(status).not_to be_success
    expect(stderr).to match(/SHOPIFY_SHOP_DOMAIN/)
  end
end
