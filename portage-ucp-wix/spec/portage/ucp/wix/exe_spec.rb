...
RSpec.describe "exe/portage-ucp-wix" do
  let(:gem_root) { File.expand_path("../../../..", __dir__) }
  let(:exe_path) { File.join(gem_root, "exe", "portage-ucp-wix") }
  let(:base_env) do
    { "WIX_ACCESS_TOKEN" => "wix-tok" }
  end
  ...
  # answers requests, without needing a real Wix site.
  ...
  it "raises a clear error when a required env var is missing" do
    _stdout, stderr, status = run_exe({ "WIX_ACCESS_TOKEN" => nil }, stdin_data: "")

    expect(status).not_to be_success
    expect(stderr).to match(/WIX_ACCESS_TOKEN/)
  end
end
