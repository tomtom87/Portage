...
RSpec.describe "exe/portage-ucp-woocommerce" do
  let(:gem_root) { File.expand_path("../../../..", __dir__) }
  let(:exe_path) { File.join(gem_root, "exe", "portage-ucp-woocommerce") }
  let(:base_env) do
    {
      "WOOCOMMERCE_SITE_URL" => "https://example.test",
  ...
  # answers requests, without needing a real WooCommerce store.
  ...
  it "raises a clear error when a required env var is missing" do
    _stdout, stderr, status = run_exe({ "WOOCOMMERCE_SITE_URL" => nil }, stdin_data: "")

    expect(status).not_to be_success
    expect(stderr).to match(/WOOCOMMERCE_SITE_URL/)
  end
end
