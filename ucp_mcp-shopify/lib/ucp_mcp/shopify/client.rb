...
module UcpMcp
  module Shopify
    ...
    # data it needs (see UcpMcp::Shopify::Adapter).
    class Client
      ...
      def require_token!(token, name)
        raise ArgumentError, "UcpMcp::Shopify::Client requires #{name} for this call" unless token
      end
      ...
      def post(path, headers:, query:, variables:)
        ...
        raise UcpMcp::Shopify::GraphqlError, body["errors"] if body["errors"]
