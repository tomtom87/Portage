...
RSpec.describe UcpMcp::CapabilityRegistry do
  ...
  it "advertises only capabilities whose adapter methods are overridden" do
    catalog_only = Class.new(UcpMcp::Adapter) do
  ...
  it "advertises nothing for a plain, unconfigured Adapter" do
    expect(registry.advertised(UcpMcp::Adapter.new)).to eq([])
  end
  ...
  describe ".default" do
    it "includes the built-in catalog/cart/checkout/order/identity capabilities" do
